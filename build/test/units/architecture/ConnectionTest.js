...
describe("Connection", function () {
    const from = new Node_1.Node();
    const to = new Node_1.Node();
    const weight = Utils_1.randDouble(-1, 1);
    const gateNode = new Node_1.Node();
    const gain = Utils_1.randDouble(-1, 1);
    describe("new Connection()", function () {
        it(`new Connection(from=${from}, to=${to}) => {Connection}`, function () {
            const connection = new Connection_1.Connection(from, to);
        ...
        it(`new Connection(from=${from}, to=${to}, weight=${weight}) => {Connection}`, () => {
            const connection = new Connection_1.Connection(from, to, weight);
            chai_1.expect(connection).to.be.an.instanceOf(Connection_1.Connection);
